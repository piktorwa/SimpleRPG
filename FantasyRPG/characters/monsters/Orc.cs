...
{
    internal class Orc : ICharacter
    {
        public Orc(int Turn)
        {
            _strength = 10 + 2 * Turn;
            _endurance = (int)(2 + 1.5 * Turn);
            _swiftness = (int)(4 + Turn * 0.8);
            _accuracy = (int)(170 + Turn * 0.1);
            _maxHP = _endurance * 45;
            _HP = _maxHP;
            _armor = (int)(10 + Turn * 1.5);
        }
        private string _name = "Orc";
        public string Name
        {
            get
            {
        ...
        public int Experience { get; set; } = 250;
        public int Level
        {
            get;
            set;
