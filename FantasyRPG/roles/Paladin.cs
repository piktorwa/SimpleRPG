...
{
    internal class Paladin:IRole
    {
        public string Name { get; } = "Paladin";
        public Paladin(ICharacter race)
        {
            _self = race;
            _self.Armor = 15;
            IStates NewState = new Regeneration();
            _self.AddState(NewState);
        }
        ...
        public void Attack(IRole defender)
        {
            ...
            if (NoMiss.Next(100) < _self.Accuracy - defender.Self.Swiftness)
            {
                ...
                if (defender.Self.Armor < (int)(_self.Strength * 2.5))
                {
                    defHP = (int)(defHP - (_self.Strength * 2.5 - defender.Self.Armor));
